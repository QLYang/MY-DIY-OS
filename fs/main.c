...
PRIVATE void mkfs()
{
	...
	pi->i_size = DIR_ENTRY_SIZE * 5; /* 4 files:
					  * `.',
					  * `dev_tty0', `dev_tty1', `dev_tty2','cmd.tar'
					  */
	pi->i_start_sect = sb.n_1st_sect;
	pi->i_nr_sects = NR_DEFAULT_FILE_SECTS;
	/* inode of `/dev_tty0~2' */
	for (i = 0; i < NR_CONSOLES; i++) {
		pi = (struct inode*)(fsbuf + (INODE_SIZE * (i + 1)));/*first inode is root(skip),and then tty1~3*/
